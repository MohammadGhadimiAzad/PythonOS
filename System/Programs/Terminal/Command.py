...
# -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def shutdown_command(command):
        ...
        GlobalObjects().os.destroy()
        sig = getattr(signal, "SIGKILL", signal.SIGTERM)
        os.kill(os.getpid(), sig)
        

    if command.startswith("shutdown"):
        master.after(1000, shutdown_command(command))
    ...
    def curl(command):
        ...
        r = requests.get(f'http://localhost:{GlobalObjects().port}/{data[2]}')
        res = r.text
    ...
    if command.startswith("curl"):
        master.after(1000, curl(command))


    def ipconfig_command(command):
        ...
        data = command.split(" ")
        if (len(data) == 1):
            output.insert(INSERT, GlobalObjects().ip + "\n")
            output.see(END)
        else:
            ip = data[1]
            GlobalObjects().save_setting(ip)
            output.insert(INSERT, GlobalObjects().ip + "\n")
            output.see(END)

    if command.startswith("ipconfig"):
        master.after(1000, ipconfig_command(command))
...
# -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def help_command():
        ...
        output.insert(INSERT, 'shutdown                 - shutdown os' +                                      '\n')
        output.insert(INSERT, 'ipconfig                 - show & set pc ip' +                                 '\n')
        output.insert(INSERT, 'curl 1.1.1.1 5000        - service call by ip and port number' +               '\n')
